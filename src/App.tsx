...
function CodeRoom() {
  ...
  const handleJoinRoom = useCallback(() => {
    ...
    // We'll handle the actual joining logic in the editor component
    setView("editor");
  }, [joinCode]);
  ...
  if (view === "selection") {
    return (
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md mx-auto">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold accent-text mb-4">Choose an Option</h1>
          </div>
          
          <div className="space-y-6">
            <button
              onClick={handleCreateRoom}
              className="w-full bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg px-4 py-3 text-lg font-medium transition-colors"
            >
              Create a Room
            </button>
  ...
  return <CodeEditor initialRoomId={selectedRoomId} roomCode={joinCode} onBack={() => setView("selection")} />;
}

function CodeEditor({ initialRoomId, roomCode, onBack }: { 
  ...
  }, [roomCode, joinRoomByCode, onBack]);
    ...
    <>
      ...
      <div className="flex-1 flex flex-col">
        {selectedRoomId ? (
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <p className="text-xl mb-4">Create a new room or select an existing one</p>
              <button
                onClick={() => void handleCreateRoom()}
                className="bg-indigo-500 text-white rounded px-6 py-3 text-lg"
              >
                Create New Room
              </button>
            </div>
